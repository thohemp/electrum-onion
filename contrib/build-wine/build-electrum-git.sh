...
pushd $WINEPREFIX/drive_c/electrum-onion
...
pushd ./contrib/deterministic-build/electrum-onion-locale
if ! which msgfmt > /dev/null 2>&1; then
    fail "Please install gettext"
fi
for i in ./locale/*; do
    dir=$WINEPREFIX/drive_c/electrum-onion/electrum_onion/$i/LC_MESSAGES
    mkdir -p $dir
...
cd dist
mv electrum-onion-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
